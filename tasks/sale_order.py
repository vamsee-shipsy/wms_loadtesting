        ...
        with self.client.post("/api/v1/outbound/orders/", headers=headers, data=payload, catch_response=True, name='order_creation') as sync_response:
            try:
                ...
                if sync_response.status_code != 200:
                    self.user.logger.error(f"Failed to create sale order through sync, error response : {response_data} in warehouse: {warehouse} with access token: {token_data.get('access_token')}")
                    sync_response.failure(f"Failed to create sale order through sync, {response_data}")
                else:
                    self.user.logger.info(f"Sale order created successfully with warehouse: {warehouse}")

            except Exception as e:
                self.user.logger.error(f"Failed to create sale order through sync, error response : {sync_response.text} in warehouse: {warehouse} with access token: {token_data.get('access_token')}")
        ...
        with self.client.post("/api/v1/async/outbound/orders/", headers=self.headers, data=payload, catch_response=True, name='async_order_creation') as async_response:
            try:
                ...
                if response_data.get('id'):
                    ...
                    self.user.logger.info(f"Sale order created successfully with warehouse: {warehouse} with id: {self.uuid}")
                else:
                ...
                self.user.logger.error(f"Failed to create sale order through async, error response : {async_response.text} in warehouse: {warehouse} with access token: {token_data.get('access_token')}")
